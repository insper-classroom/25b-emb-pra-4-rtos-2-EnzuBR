#include "FreeRTOS.h"
#include "task.h"
#include "semphr.h"
#include "queue.h"

#include <stdio.h>
#include "pico/stdlib.h"
#include "pico/time.h"

#include "hardware/gpio.h"
#include "hardware/i2c.h"
#include "pins.h"
#include "ssd1306.h"

// === Definições para SSD1306 ===
ssd1306_t disp;

QueueHandle_t xQueueTime;
QueueHandle_t xQueueDistance;
SemaphoreHandle_t xSemaphoreTrigger;

// == funcoes de inicializacao ===
void pin_callback(uint gpio, uint32_t events) {
    absolute_time_t time = get_absolute_time();
    xQueueSendFromISR(xQueueTime, &time, 0);
}

void oled_display_init(void) {
    i2c_init(i2c1, 400000);
    gpio_set_function(2, GPIO_FUNC_I2C);
    gpio_set_function(3, GPIO_FUNC_I2C);
    gpio_pull_up(2);
    gpio_pull_up(3);

    disp.external_vcc = false;
    ssd1306_init(&disp, 128, 64, 0x3C, i2c1);
    ssd1306_clear(&disp);
    ssd1306_show(&disp);
}

void led_rgb_init(void) {
    gpio_init(LED_PIN_R);
    gpio_set_dir(LED_PIN_R, GPIO_OUT);
    gpio_put(LED_PIN_R, 1);

    gpio_init(LED_PIN_G);
    gpio_set_dir(LED_PIN_G, GPIO_OUT);
    gpio_put(LED_PIN_G, 1);

    gpio_init(LED_PIN_B);
    gpio_set_dir(LED_PIN_B, GPIO_OUT);
    gpio_put(LED_PIN_B, 1);
}

void ultrassom_init(void) {
    gpio_init(TRIG_PIN);
    gpio_set_dir(TRIG_PIN, GPIO_OUT);
    gpio_put(TRIG_PIN, 0);

    gpio_init(ECHO_PIN);
    gpio_set_dir(ECHO_PIN, GPIO_IN);

    gpio_set_irq_enabled_with_callback(ECHO_PIN, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL, true, &pin_callback);
}

void trigger_task(void *p) {
    while (1) {
        gpio_put(TRIG_PIN, 1);
        sleep_us(10);
        gpio_put(TRIG_PIN, 0);
        xSemaphoreGive(xSemaphoreTrigger);
        vTaskDelay(pdMS_TO_TICKS(200));
    }
}

void echo_task(void *p) {
    absolute_time_t start_time, end_time;
    while (1) {
        if (xQueueReceive(xQueueTime, &start_time, portMAX_DELAY)) {
            if (xQueueReceive(xQueueTime, &end_time, pdMS_TO_TICKS(50))) {
                uint64_t duration = absolute_time_diff_us(start_time, end_time);
                double distance = (double)duration * 0.0343 / 2.0;
                xQueueSend(xQueueDistance, &distance, 0);
            }
        }
    }
}

void oled_task(void *p) {
    double distance;
    char buffer[20];

    while (1) {
        xSemaphoreTake(xSemaphoreTrigger, portMAX_DELAY);

        if (xQueueReceive(xQueueDistance, &distance, pdMS_TO_TICKS(100))) {
            ssd1306_clear(&disp);
            snprintf(buffer, sizeof(buffer), "Dist: %.1f cm", distance);
            ssd1306_draw_string(&disp, 0, 16, 2, buffer);

            int bar_width = (int)(distance / 200.0 * 128);
            if (bar_width > 128) bar_width = 128;
            if (bar_width < 0) bar_width = 0;
            ssd1306_draw_square(&disp, 0, 40, bar_width, 10);

            ssd1306_show(&disp);

            if (distance <= 100.0) {
                gpio_put(LED_PIN_G, 0);
                gpio_put(LED_PIN_R, 1);
            } else {
                gpio_put(LED_PIN_G, 0);
                gpio_put(LED_PIN_R, 0);
            }
            gpio_put(LED_PIN_B, 1);

        } else {
            ssd1306_clear(&disp);
            ssd1306_draw_string(&disp, 0, 24, 2, "Falha no");
            ssd1306_draw_string(&disp, 0, 40, 2, "sensor!");
            ssd1306_show(&disp);

            gpio_put(LED_PIN_R, 0);
            gpio_put(LED_PIN_G, 1);
            gpio_put(LED_PIN_B, 1);
        }
    }
}

int main() {
    stdio_init_all();

    oled_display_init();
    led_rgb_init();
    ultrassom_init();

    xQueueTime = xQueueCreate(10, sizeof(absolute_time_t));
    xQueueDistance = xQueueCreate(5, sizeof(double));
    xSemaphoreTrigger = xSemaphoreCreateBinary();

    xTaskCreate(trigger_task, "Trigger Task", 256, NULL, 1, NULL);
    xTaskCreate(echo_task, "Echo Task", 256, NULL, 2, NULL);
    xTaskCreate(oled_task, "OLED Task", configMINIMAL_STACK_SIZE + 512, NULL, 1, NULL);

    vTaskStartScheduler();

    while (true);
}